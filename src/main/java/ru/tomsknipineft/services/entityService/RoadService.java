...
public class RoadService implements EntityProjectService {
    ...
    public Integer getResourceForEngSurveyRoad(Road road){
        if (road.isActive()){
            this.findRoadFromRequest = roadRepository.findFirstByCategoryAndLengthGreaterThanEqualAndCountGreaterThanEqual(road.getCategory(),
                    road.getLength(), road.getCount()).orElseThrow(()-> new NoSuchEntityException("Введены некорректные значения параметров автодороги " +
                    road.getCategory() + " и " + road.getLength()+ " и " + road.getCount()));
