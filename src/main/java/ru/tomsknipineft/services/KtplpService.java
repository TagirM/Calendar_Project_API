...
public class KtplpService {
    ...
    public Integer getResourceForEngSurveyKtplp(Ktplp ktplp){
        if (ktplp.isActive()){
            this.findKtplpFromRequest = ktplpRepository.findByKtplpType(ktplp.getKtplpType()).orElseThrow(()->
                    new NoSuchEntityException("Введено некорректное значение " + ktplp.getKtplpType()));
